...
# Usage (example)
# ---------------------------------------------------------
# python cnn_lstm_crypto.py --csv "Bitcoin Historical Data.csv" \
#   --target Price --features Open High Low "Vol." "Change %" \
#   --seq-len 7 --epochs 60 --conv-channels 64 --kernel-size 3 \
#   --test-start 08/14/2025 --test-end 08/21/2025
# =========================================================
...
class CNNLSTMForecaster(nn.Module):
    """
    Input:  x [B, T, F]
    Conv1d over time (channels = features) -> [B, C, T]
    LSTM over time with conv channels as features -> [B, T, H]
    FC on last step -> [B, 1]
    """
    def __init__(self, input_size, conv_channels=32, kernel_size=3,
                 lstm_hidden=64, lstm_layers=2, dropout=0.2):
        super().__init__()
        pad = kernel_size // 2  # keeps T the same for odd kernel sizes
...
# -----------------------------
def main():
    parser = argparse.ArgumentParser(description="CNN+LSTM for cryptocurrency forecasting (PyTorch)")
    ...
    parser.add_argument("--train-split", type=float, default=0.8,
                        help="Fraction for time-based split (ignored if --test-start and --test-end provided).")
    ...
    # CNN params
    parser.add_argument("--conv-channels", type=int, default=32, help="Number of Conv1d output channels.")
    parser.add_argument("--kernel-size", type=int, default=3, help="Temporal kernel size (odd number recommended).")
    ...
    if args.test_start and args.test_end:
        ...
        # Train: strictly before test_start; Test: [start, end]
        ...
        split_idx = max(split_idx, min_seq_rows)  # ensure training has one sequence
        if n - split_idx < min_seq_rows:          # ensure test has one sequence
    ...
    # Test sequences across the boundary: prepend last seq_len train rows as context
    ...
    if len(Xtr_seq) == 0 or len(Xte_seq) == 0:
        raise ValueError(
            f"After splitting and sequencing, got {len(Xtr_seq)} train sequences and {len(Xte_seq)} test sequences. "
    ...
    # Plot (markers + daily ticks for short windows)
    ...
    plt.title(f"CNN+LSTM Forecast (Test Period){title_extra}")
    plt.xlabel("Date")
    ...
    model_path = Path("cnn_lstm_crypto.pt")
    torch.save({
